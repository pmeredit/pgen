...
impl AggNormalize for Expr {
    fn agg_normalize(self) -> Result<Box<Self>, String> {
        ...
        match self {
            ...
            Array(a) => {
                let a: Result<Vec<Box<Expr>>, String> =
                    a.into_iter().map(|e| e.agg_normalize()).collect();
                Ok(Box::new(Array(a?)))
            }
            App(s, args) => {
                ...
                let args: Result<Vec<Box<Expr>>, String> =
                    args.into_iter().map(|e| e.agg_normalize()).collect();
                let args = args?;
                ...
                match AGG_FUNCTIONS[s.as_str()].arity {
                    ...
                    Variadic(i) => {
                        if args_len < i {
                            return Err(format!(
                                "'{}' takes at least {} arguments, but {} provided",
                                s, i, args_len
                            ));
                        }
                    }
                    Optional(min, max) => {
                        if args_len < min || args_len > max {
                            return Err(format!(
                                "'{}' takes between {} and {} arguments, but {} provided",
                                s, min, max, args_len
                            ));
                        }
                    }
...
fn check_vars(stage: &Expr, i: usize, stage_name: &str) -> Result<(), String> {
    ...
    if fvs.len() != 0 {
        return Err(format!(
            "Stage: {} with Type: '{}' contains undefined variables: {:?}",
            i, stage_name, fvs
        ));
    }
    Ok(())
    ...
    fn normalize(self) -> Result<Box<Self>, String> {
        ...
        for (i, PipelineItem { stage_name, stage }) in self.stages.into_iter().enumerate() {
            if !STAGES.contains(stage_name.as_str()) {
                return Err(format!("{} is not a valid mongo stage", stage_name));
            }
            if stage_name == "match" {
                check_vars(&*stage, i, &stage_name)?;
                if let Expr::Object(_) = *stage {
                    stages.push(PipelineItem {
                        stage_name,
                        stage: stage.match_normalize()?,
                    });
                } else {
                    stages.push(PipelineItem {
                        stage_name,
                        stage: Box::new(Expr::App(
                            "expr".to_string(),
                            vec![stage.agg_normalize()?],
                        )),
                    });
                }
                ...
                stages.push(PipelineItem {
                    stage_name,
                    stage: stage.agg_normalize()?,
                });
            } else {
                let variant = stage.variant();
                let article = if set!['I', 'A', 'O'].contains(&variant.chars().nth(0).unwrap()) {
                    "an"
                } else {
                    "a"
                };
                return Err(format!(
                    "Stage: {} with Type: '{} must contain an Object not {} {}",
                    i, stage_name, article, variant
                ));
            }
        }
...
impl AggNormalize for Map {
    fn agg_normalize(self) -> Result<Box<Self>, String> {
        Ok(Box::new(Map {
            input: self.input.agg_normalize()?,
            ename: self.ename,
            expr: self.expr.agg_normalize()?,
        }))
    }
...
impl AggNormalize for Zip {
    fn agg_normalize(self) -> Result<Box<Self>, String> {
        ...
        match *inputs {
            Expr::Array(ref v) => match defaults {
                None => (),
                Some(ref d) => match d.as_ref() {
                    &Expr::Array(ref a) => {
                        if v.len() != a.len() {
                            return Err(format!(
                                "Inputs and defaults for zip must have same size, sizes are {} \
                                 and {} respectively",
                                v.len(),
                                a.len()
                            ));
                    ...
                    _ => {
                        return Err(format!(
                            "Defaults for zip must be an array, not {:?}",
                            defaults
                        ))
                    }
                },
            ...
            _ => {
                return Err(format!(
                    "Inputs to zip must be an array, not {}",
                    inputs.variant()
                ))
            }
        };
...
impl MatchNormalize for Expr {
    fn match_normalize(self) -> Result<Box<Self>, String> {
        ...
        match self {
            ...
            Array(a) => {
                let a: Result<Vec<Box<Expr>>, String> =
                    a.into_iter().map(|e| e.match_normalize()).collect();
                Ok(Box::new(Array(a?)))
            }
            App(s, args) => {
                ...
                let args: Result<Vec<Box<Expr>>, String> =
                    args.into_iter().map(|e| e.match_normalize()).collect();
                let args = args?;
                let args_len = args.len();
